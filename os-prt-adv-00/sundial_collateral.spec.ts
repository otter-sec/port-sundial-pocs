...
describe('SundialCollateral', () => {
  ...
  it('Liquidation (borderline case, exactly amount of loan that would be liquidated)', async () => {
    ...
    const updatePriceAndPrepareLiquidation = async () => {
      ...
      const beforeCollateralValue = sundialProfileWrapper.getTotalCollateralValue();
      console.log("total collateral value before: " + beforeCollateralValue);
      console.log("total loan value before:       " + sundialProfileWrapper.getTotalLoanValue());

      const saberRepayVault = await createTokenAccount(provider, saberMint, sdk.provider.wallet.publicKey);

      for (let i=0;i<200;i++) {
        if (i % 20 == 0) {
          const tx = new Transaction();
          await updateOraclesSlot();
          tx.add(refreshReserveInstruction(serumReserveState.address, serumOracleKP.publicKey));
        ...
        const transferTx = new TransactionEnvelope(
          sdk.provider,
          [transfer({source: saberVault, destination: saberRepayVault, amount: 1, owner: sdk.provider.wallet.publicKey})],
        );
        expectTX(transferTx).to.be.fulfilled;
        await sleep(20);
        expectTX(await sundialProfileWrapper.liquidateSundialProfile(
          sundialSerumCollateralWrapper,
          sundialSaberWrapper,
          saberRepayVault,
        )).to.be.fulfilled;
      ...
      await sundialProfileWrapper.reloadData();
      const afterCollateralValue = sundialProfileWrapper.getTotalCollateralValue();
      console.log("total collateral value after:  " + afterCollateralValue);
      console.log("total loan value:              " + sundialProfileWrapper.getTotalLoanValue());

      const tx = await sundialProfileWrapper.liquidateSundialProfile(
        sundialSerumCollateralWrapper,
        sundialUSDCWrapper,
        usdcVault,
      );
